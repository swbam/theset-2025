...
DROP FUNCTION IF EXISTS initialize_show_setlist(UUID, JSONB);
DROP FUNCTION IF EXISTS add_song_to_setlist(UUID, TEXT, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS cast_setlist_vote(UUID, UUID, TEXT);

-- Function to initialize a show setlist with Spotify top tracks
CREATE FUNCTION initialize_show_setlist(
  p_show_id UUID,
...
CREATE FUNCTION add_song_to_setlist(
  p_setlist_id UUID,
  p_song_name TEXT,
  p_spotify_id TEXT,
...
CREATE FUNCTION cast_setlist_vote(
